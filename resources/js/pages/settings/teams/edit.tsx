...
export default function TeamsEdit({ team }: Props) {
  ...
  const updateRole = (userId: number, role: string) => {
    router.patch(`/settings/teams/${team.id}/members/${userId}`, { role }, {
      preserveScroll: true,
    })
  }
  ...
  return (
    <>
      ...
      <Heading level={2} className="mb-6">Edit Team</Heading>
      ...
      <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <Form validationErrors={teamForm.errors} onSubmit={submitTeam} className="max-w-lg space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Form validationErrors={memberForm.errors} onSubmit={submitMember} className="flex max-w-lg items-end gap-3">
                <TextField value={memberForm.data.email} onChange={(v) => memberForm.setData("email", v)} className="flex-1">
                  <Label>Add Member</Label>
                  <Input type="email" placeholder="user@example.com" />
                  <FieldError>{memberForm.errors.email}</FieldError>
                </TextField>
                <Select selectedKey={memberForm.data.role} onSelectionChange={(v) => memberForm.setData("role", v as string)}>
                  <Label>Role</Label>
                  <SelectTrigger className="w-28" />
                  <SelectContent
                    items={[
